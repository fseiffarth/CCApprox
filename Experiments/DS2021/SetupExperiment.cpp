//
// Created by florian on 25.08.21.
//
...
void SetupExperiment::getClosedFromApproximation(std::vector<NodeId>& approximation, std::set<NodeId>& closedSet, int iterations, double threshold){
    closedSet.clear();
...
void SetupExperiment::SamplingApproximation(GraphClosureSP &gc, GraphData &graph, std::vector<GraphData>& subgraphData, std::vector<NodeId>& neighborIds, OuterplanarSubgraphDFS* outerPlanarSubgraphDfs,
    ...
    if (subgraphData.size() == 1) {
        ...
        gc.approx_closure(subgraphData, closureParameters, approximation, closure_time);
