...
void closure_runtime() {
    ...
    for (int j = 0; j < 8; ++j) {
        ...
        for (auto const &size: graph_sizes) {
            ...
            FileEvaluation fileEvaluation = FileEvaluation("../out/Experiments/OuterplanarRuntime/" + out_name);
            OuterplanarGraphStatistics statistics;
            std::vector<std::string> values;
            outerplanar.clear();
            graphs.clear();
            trees.clear();
            Experiments::GetOuterplanarSamplesBiconnected(size, 10, outerplanar, graphs, trees);
            int edges = 0;
            ...
            statistics /= (int) outerplanar.size();
