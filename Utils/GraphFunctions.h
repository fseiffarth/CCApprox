...
public:
    ...
    static void GetCoreGraphStats(GraphData& core_graph, GraphData &graph, TIntV &coreNodes, double runtime, const std::string& core_name = "Core", FileEvaluation* fileEvaluation = nullptr);
    ...
    static void GetOuterplanarSamples(GraphData &graph, PatternType type, std::vector<OuterplanarGraphData> &subgraphs,
                           OuterplanarSubgraphDFS *outerplanarSubgraphDFS, std::vector<NodeId>& neighborIds, int samples, int seed, double& runtime, double &conversion_runtime, bool clear = false);
