...
void GraphFunctions::GetSamples(GraphData &graph, PatternType type, std::vector<GraphData> &subgraphs,
                                 OuterplanarSubgraphDFS *outerplanarSubgraphDFS, std::vector<NodeId>& neighborIds, int samples, int seed, double& runtime) {
    runtime = 0;
    subgraphs.clear();
    ...
    for (int j = 0; j < samples; ++j) {
        ...
        switch (type) {
            case PatternType::BFS_TREE:
                ...
                GraphFunctions::bfsSubtree(graph.get_graph(), subgraphs.back(), neighborIds, generator);
                subgraphs.back().graphType = GraphType::TREE;
                std::cout << "Generated tree sample " << j << "/" << samples << " of " << graph.getName() << std::endl;
                break;
            case PatternType::OUTERPLANAR:
                subgraphs.emplace_back(GraphData(new TUNGraph, (int) graph.size()));
                outerplanarSubgraphDFS->generate(subgraphs.back(), generator, false);
                subgraphs.back().graphType = GraphType::OUTERPLANAR;
...
void GraphFunctions::GetOuterplanarSamples(GraphData &graph, PatternType type, std::vector<OuterplanarGraphData> &subgraphs,
                                           ...
                                           std::vector<NodeId> &neighborIds, int samples, int seed, double &runtime, double &conversion_runtime, bool clear) {
    runtime = 0;
    ...
    for (int j = 0; j < samples; ++j) {
        ...
        subgraphs.back().set();
        if(clear) {
            subgraphs.back().graph().Clr();
            subgraphs.back().ContainmentList.clear();
            subgraphs.back().DistanceList.clear();
            subgraphs.back().neighborList.clear();
            subgraphs.back().PredecessorsList.clear();
        }
...
void GraphFunctions::GetCoreGraphStats(GraphData& core_graph, GraphData &graph, TIntV &coreNodes, double runtime, const std::string& core_name, FileEvaluation* fileEvaluation) {
    //Statistics of core graph
    core_graph = GraphData(TSnap::GetSubGraph(graph.get_graph(), coreNodes));
    ...
    if (fileEvaluation != nullptr){
        ...
        fileEvaluation->headerValueInsert({core_name + " Nodes", core_name + " Relative Nodes", core_name + " Edges", core_name + " Relative Edges", core_name + " Out Edges"},
                                          {std::to_string(core_graph.size()), std::to_string((double) core_graph.size()/(double) graph.size()),
                                           ...
                                           std::to_string(overlapOutEdges)}, -1);
    }
