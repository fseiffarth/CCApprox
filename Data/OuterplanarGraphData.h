//
// Created by florian on 07.11.21.
//
...
public:
    ...
    OuterplanarGraphData(PUNGraph pGraph) : GraphData(pGraph){
        nodeToComponents = std::vector<std::vector<int>>(pGraph->GetNodes());
        maxCompSize = std::vector<int>(pGraph->GetNodes(), 0);
        set();
    };
    OuterplanarGraphData(GraphData& graphData) : GraphData(graphData){
